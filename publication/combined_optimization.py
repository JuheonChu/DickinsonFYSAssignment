...
blended_obj_functions = sum([obj_coef[1] * f_rank, obj_coef[2] * f_gender])


model.setObjective(blended_obj_functions, GRB.MINIMIZE)

model.optimize()

blended_optimal_value = model.getObjective().getValue()

# Add constraint to maintain optimal value (e.g. if blended_obj_function >= 0, we can do <= 0.95 <= blended_optimal_value)
model.addConstr(obj_coef[1] * f_rank + obj_coef[2]*f_gender <= blended_optimal_value, "Hierarchical degradation")

model.setObjective(f_stu_type, GRB.MINIMIZE)
